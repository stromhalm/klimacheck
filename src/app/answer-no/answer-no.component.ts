...
@Component({
  selector: 'app-answer-no',
  templateUrl: './answer-no.component.html',
  styleUrls: ['./answer-no.component.less'],
  animations: [
    trigger('scale', [
      transition(':enter', [
        style({
...
export class AnswerNoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.formattedAnswer = this.mdPipe.transform(this.question['false'])
    .replace('  ', '</p><p>')
    .replace('  ', '</p><p>')
    .replace('  ', '</p><p>')
